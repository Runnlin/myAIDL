...
abstract class IBookManagerStub() : IBookManager, Binder() {
    ...
    override fun onTransact(code: Int, data: Parcel, reply: Parcel?, flags: Int): Boolean {
        when (code) {
            ...
            TRANSACTION_addBook -> {
                data.enforceInterface(DESCRIPTOR)
                val arg0: Book? = if (0 != data.readInt()) {
    ...
    companion object {
        ...
        class Proxy(remote: IBinder) : IBookManager {
            ...
            override fun getBookList(): List<Book> {
                ...
                val _result: ArrayList<Book>

                try {
                    _data.writeInterfaceToken(DESCRIPTOR)
                    mRemote.transact(TRANSACTION_getBookList, _data, _reply, 0)
                    _reply.readException()
                    _result = _reply.createTypedArrayList<Book>(Book.CREATOR)!!
                } finally {
                    _reply.recycle()
                    _data.recycle()
                }
        ...
        const val TRANSACTION_getBookList = FIRST_CALL_TRANSACTION + 0
        const val TRANSACTION_addBook = FIRST_CALL_TRANSACTION + 1
        const val TRANSACTION_registerListener = FIRST_CALL_TRANSACTION + 2
        const val TRANSACTION_unregisterListener = FIRST_CALL_TRANSACTION + 3
    }
}